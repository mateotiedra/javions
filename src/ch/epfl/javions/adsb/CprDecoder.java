...
public class CprDecoder {
    ...
    public static GeoPos decodeCprPosition(double x0, double y0, double x1, double y1, int mostRecent) {
        Preconditions.checkArgument(mostRecent == 0 || mostRecent == 1);
        final int Zphi0 = 60;
        final int Zphi1 = 59;
        final double majDeltaPhi0 = 1 / Zphi0;
        final double majDeltaPhi1 = 1 / Zphi1;
        ...
        double A = Math.acos(1 - (1 - Math.cos(2 * Math.PI * majDeltaPhi0) / Math.pow((Math.cos(phi0)), 2)));
        ...
        if ((Units.convert(phi0, Units.Angle.T32, Units.Angle.DEGREE) >= 90 && Units.convert(phi0, Units.Angle.T32, Units.Angle.DEGREE) <= -90)
                ...
                || zphi0 != zphi1)
            return null;
        else if (mostRecent == 0) return new GeoPos((int) phi0, (int) delta0);
        else return new GeoPos((int) phi1, (int) delta1);
    }
}
